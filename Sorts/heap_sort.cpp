...
#include <sstream>
using namespace std;
typedef int Data_t;  // тип элемента в куче

// ввод списка значений в вектор без терминального элемента
// из одной строки с разделителем-пробелом
vector<Data_t> input_vector();

// вывод значений вектора через пробел в одной строке
...
void heap_sort(vector<Data_t> &A)
{
    ...
    for (size_t k = 1; k < A.size(); k++) {
        // подъём очередного на вершину пирамиды.
        size_t i = k;
    ...
    {
        swap(A[0], A[top]);
        // последний - наверх, максимальный - в конец
