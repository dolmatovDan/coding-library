...
int main()
{
    ...
    int Q_end = 0; //Номер элемента, который будет выходить из очереди Q
    ...
    for (int i = Q_size; i < N; i++) {
        ...
        //PushPop в цикл. очередь
        int number_leaving_queue = Q[Q_end];
        Q[Q_end++] = current;
        if (Q_end > Q_size) Q_end = 0;

        //Для ного current новое число стало возможным парным, и поэтому рассматриваем его
